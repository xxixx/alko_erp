<template>
  <div>
    ...
    checkedWorkers{{ savedWorkers }}
    selectedDate{{ selectedDate }}
    <hr>
    <!-- dayOfworkers{{ dayOfworkers }} -->
    <hr>
    <br>
    dayOfworkersComputed{{ dayOfworkersComputed }}
    <!-- dayOfworkers{{ dayOfworkers }} -->
    <div>
  ...
  <table>
    ...
    <tbody>
      <tr v-for="worker in dayOfworkersComputed" :key="worker.NO">
        <td>{{ worker.NO }}{{ worker.WorkerNames }}</td>
...
const fetchDataByDate = async () => {
  ...
  try {
    ...
    const result = await $fetch<WorkerModel[]>(`/api/workers/getWorkersDate?date=${selectedDate.value}`);
