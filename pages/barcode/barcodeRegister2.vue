...
const handleBarcodeInput = async () => {
  ...
  // 바코드 중복 확인
  if (scannedBarcodes.value.includes(scannedBarcode.value)) {
    alert('이미 등록된 바코드입니다.')
    return
  }
  ...
  // 서버로 바코드 등록 요청
  try {
    await  fetch('/barcode/BarcodeScanRegister', {
      barcode: scannedBarcode.value,
      processCode: 'PROCESS_CODE', // 필요 시 다른 프로세스 코드 설정
      boxNo: getBoxNo(),
      createDate: new Date().toISOString(),
    })
    
    // 바코드 저장 후 배열에 추가
    scannedBarcodes.value.push(scannedBarcode.value)
    scannedBarcode.value = ''
  } catch (error) {
    console.error('Error during barcode registration:', error)
  }
}
