...
const fetchPagedData = async (page = 1) => { // 기본값을 1로 설정
  ...
  const { data, totalPages: fetchedTotalPages } = await $fetch(`/api/jaedan/getPagedRecord?page=${page}&pageSize=${pageSize}`);
  isLoading.value = false;
  console.log("data", data);
  console.log("totalRecords", totalPages.value); // ref 객체의 값을 가져옵니다.
  console.log("data length", data.length);
  
  jaedanRecord.value = data; // 'data' 필드에서 실제 레코드 가져오기
...
const fetchDataByDateRange = async () => {
  ...
  try {
    const { data, page, limit, totalPages: fetchedTotalPages } = await $fetch(`/api/jaedan/getRecordsByDate?startDate=${startDate.value}&endDate=${endDate.value}&page=${currentPage.value}&limit=${pageSize}`);
...
const fetchDataSerch = async () => {
  ...
  const { data, page, limit, totalPages: fetchedTotalPages } = await $fetch(`/api/jaedan/searchRecords?searchTerm=${searchTerm.value}&page=${currentPage.value}&limit=${pageSize}`);
