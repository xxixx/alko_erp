<template>
  <div class="w-full max-w-3xl mx-auto py-8">
    ...
    <!-- partJaedanOrderList{{partJaedanOrderList}} -->
    <!-- getOrderStateUp{{partJadanOrder}} -->
    <!-- selectOrder{{selectOrder}} -->
    <!-- getPartJaedanRecord10{{getPartJaedanRecord10}} -->
    <!-- partJaedanRecord{{partJaedanRecord}} -->
    <div v-if="isLoading">
    ...
    <div v-else class="container mt-2">
      ...
      <P class="border bg-secondary text-white  p-2">파트 재단 리스트(작업지시서)</P>
      <table class="table table-sm text-center">
        ...
        <tbody>
          <tr v-for="(item, index) in partJaedanOrderList" :key="index">
            <td>{{ item.NO }}</td>
...
<div  class="container mt-2">
      ...
      <table class="table table-sm text-center">
        ...
        <tbody>
          <tr v-for="(item, index) in partJadanOrder" :key="index">
            <td>{{ item.NO }}</td>
              <td>{{ formatDate(item.CREATE_DATE) }}</td>
              <td>{{ item.JAEDAN_NAME }}</td>
              <td>{{ item.ASSY_PART_NAME }}</td>
            ...
            <td>
              <div v-if="item.STATE === 1">
                ...
                <div
                  class="btn btn-sm btn-warning text-white"
                  @click="onPartJaedanRegister(item)"
                >
                  완료등록
                </div>
              </div>
      ...
      <form class="">
        ...
        <div class="d-flex flex-wrap border_bottom py-1">
          ...
          >
            ...
            <span class="mx-3"> {{ PART_JAEDAN_NAME }} / </span>
            <span class="mx-3"> {{ ASSY_PART_NAME }} / </span>
    ...
    <div  class="container">
      ...
      <div class="table-responsive">
        <table class="table table-sm text-center">
          ...
          <tbody>
            <tr v-for="(item, index) in partJaedanRecord" :key="index">
              <!-- <td>{{ item.NO }}</td> -->
              <td>{{ item.JNO }}</td>
              <td>{{ item.WORK_ORDER_NO }}</td>
              <td>{{ formatDate(item.CREATE_DATE) }}</td>

              <td>{{ item.JAEDAN_NAME }}/{{ item.ASSY_PART_NAME }}</td>
...
const form = reactive({
  ...
  MARKS: "",
  COUNT: "",
  DEFECTIVE: "0", // Set default value to "0"
  CREATE_DATE: "",
  WORK_DATE: "",
  REG_ACCOUNT: parseInt(accountStore.user ? accountStore.user.NO : ''),
  PROCESSCODE: "",
});
// Y_COUNT나 MARKS가 변경될 때마다 COUNT 자동 계산
watch(
...
//파트재단
const partJaedanRecord = ref([]);
const partJaedanOrderList = ref([]);
const jaedanWondanView = ref([]);
const fetchPartJaedanList = async () => {
  try {
    const resultPartJaedanList = await $fetch("/api/workOrder/getOrderJaedanPartWithAccount");
    console.log("resultPartJaedanList data", resultPartJaedanList.data);

    partJaedanOrderList.value = resultPartJaedanList.data ;
    ...
    jaedanWondanView.value = getJaedanWondanPartData.data;
  } catch {
    alert("fetchPartJaedanList error");
  }
};

const partJadanOrder = ref([]);
const getworkOrderStateUp = async () => {
  try {
    const result = await $fetch("/api/workOrder/getPartJadanOrderWithAccountStateUp");
    console.log("getworkOrderStateUp result data", result.data);

    // form.REG_ACCOUNT = parseInt(accountStore.user ? accountStore.user.NO : "");
    partJadanOrder.value = result.data as WorkOrderModel[];
    //   const WORKING_PART_Data = await $fetch('/api/working_part/getWORKING_PART');
    //   WORKING_PART.value = WORKING_PART_Data.data as WORKING_PART_Model[];
    ...
    alert("getworkOrderStateUp Fetch error");
  }
};
...
const PART_JAEDAN_NAME = ref<string | undefined>(undefined);
const ASSY_PART_NAME = ref<string | undefined>(undefined);
...
const fetchData = async () => {
  try {
    ...
    const NO = partJaedanOrder.value;
    // const NO = route.query.param;
    // console.log("route.query.param", NO);
    const getOrder = await $fetch(`/api/workOrder/getPartJaedanOrder/${NO}`);
    ...
    if (selectOrder.value) {
      PART_JAEDAN_NAME.value = selectOrder.value.JAEDAN_NAME;
      ASSY_PART_NAME.value = selectOrder.value.ASSY_PART_NAME;
    ...
    const partJaedanRecordResult = await $fetch("/api/jaedan/getPartJaedanRecord10");
    console.log("partJaedanRecord", partJaedanRecordResult.data);
    partJaedanRecord.value = partJaedanRecordResult.data as JaedanModel[];
    ...
    alert("Fetch getPartJaedanOrder error");
  }
};
...
const insertData = async () => {
  ...
  try {
    await $fetch("/api/jaedan/partJaedanRegister", {
      method: "POST",
      body: form,
    });
    alert("등록되었습니다.");
...
const partJaedanOrder = ref();
const onPartJaedanRegister = async (item: any) => {
  // const NO = item.ACCOUNT_NAME;
  const NO = item.NO;
  console.log("NO", NO);
  partJaedanOrder.value = NO;
...
const upJadanState = async (item) => {
    const REG_NO = item.NO;
    console.log("REG_NO", REG_NO);
  await $fetch(`/api/workOrder/updateStatepartJadan/${REG_NO}`, {
    method: "POST",
    body: {
      NO: REG_NO,
      STATE: 1, // 상태를 1로 설정
...
const upJadanStateFinish = async () => {
    ...
    // console.log("REG_NO", REG_NO);
  try {
    // Update part jaedan state
    await $fetch(`/api/workOrder/updateStatepartJadan/${REG_NO.value}`, {
      method: "POST",
      body: {
        NO: REG_NO.value,
        STATE: 2,
...
const onFinish = async (item) => {
  const NO = item.JNO;
  // const WORK_DATE = formatDate(item.CREATE_DATE)
  // WORK_DATE 형식을 YYYYMMDD로 변환
  const workDate = new Date(item.CREATE_DATE);
  ...
  const PROCESSCODE = `${item.JAEDAN_NAME.replace(/\s/g, "")}${year}${month}${day}${item.NO}`;
  console.log("PROCESSCODE", PROCESSCODE);
  console.log("JaedanNo", NO);
  await $fetch(`/api/jaedan/partJaedaupdateState/${NO}`, {
    method: "POST",
    body: {
      NO: NO,
      JR_STATE: 2,
      PROCESS_STATE: 1, // 상태를 1로 설정
      UPDATE_ACCOUNT: regUserNO.value,
